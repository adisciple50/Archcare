...
def weekly_progress_report(request):
    if request.method == "POST":
        ...
        new_weekly_report = WeeklyReport(service_user=service_user_list,date=request.POST["form-0-date_year"]+"-"+request.POST["form-0-date_month"]+"-"+request.POST["form-0-date_day"],morning=request.POST["form-0-morning"],evening=request.POST["form-0-evening"],week_number=request.POST["form-0-week_number"],daily_total_one_to_one_support_hours_provided=request.POST["form-0-daily_total_one_to_one_support_hours_provided"],)
        #  new_weekly_report.clean() for date abstraction / this format: 04/12/1988
...
def daily_progress_report(request):
    # add post logic here
    if request.method == "POST":
        new_progress_report = ProgressReport(date=request.POST["form-0-date_year"]+"-"+request.POST["form-0-date_month"]+"-"+request.POST["form-0-date_day"],content_of_one_to_one_support=request.POST["form-0-content_of_one_to_one_support"],evaluation_and_comments=request.POST["form-0-evaluation_and_comments"])
...
def add_service_user(request):
    if request.method == "POST":
        new_service_user = ServiceUser(title=request.POST["title"],first_name=request.POST["first_name"],second_name=request.POST["second_name"],full_name=request.POST["full_name"],contact_phone=request.POST["contact_phone"])
